    ...
    <header className="sticky top-0 z-40 border-b border-border bg-white/95 backdrop-blur-sm">
      <Container>
        <div className="flex h-16 items-center justify-between sm:h-20">
          ...
          {/* Desktop Navigation */}
          <nav data-testid="desktop-nav" className="hidden lg:flex lg:items-center lg:gap-1">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
      ...
      >
        ...
        <div className="flex h-16 items-center justify-between px-4 sm:h-20 sm:px-6">
          ...
          >
            <span className="text-2xl">⚡</span>{" "}
            Eletroinfo <span className="text-accent-500">Regis</span>
          </Link>
          <button
            type="button"
            onClick={() => setIsMenuOpen(false)}
            className="inline-flex items-center justify-center rounded-lg p-2 text-gray-700 transition-colors hover:bg-primary-50"
        ...
        <div className="p-4">
          <Link
            ...
            rel="noopener noreferrer"
            onClick={() => setIsMenuOpen(false)}
            className="flex w-full items-center justify-center gap-2 rounded-lg bg-whatsapp px-6 py-4 text-lg font-semibold text-white transition-colors hover:bg-whatsapp-dark"
          >
            <svg
              className="h-5 w-5"
              fill="currentColor"
